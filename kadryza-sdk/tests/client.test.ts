...
import Kadryza from '../src/client'
import {
  KadryzaAuthError,
  KadryzaError,
  KadryzaNetworkError,
...
describe('Kadryza Client', () => {
  let client: Kadryza
  const originalFetch = globalThis.fetch

  beforeEach(() => {
    client = new Kadryza({
      apiKey: 'kadryza_test_xxxxxxxxxxxxxxxxxxxx',
      baseUrl: 'https://api.test.kadryza.app',
    })
  })
  ...
  describe('constructor', () => {
    it('throws if apiKey is missing', () => {
      expect(() => new Kadryza({ apiKey: '' })).toThrow(KadryzaError)
    })

    it('[C1] does not expose apiKey via console inspection', () => {
      const kadryza = new Kadryza({ apiKey: 'kadryza_live_supersecret' })
      const inspected = JSON.stringify(kadryza)
      expect(inspected).not.toContain('supersecret')
    })

    it('[M1] rejects non-HTTPS baseUrl', () => {
      expect(
        () => new Kadryza({ apiKey: 'kadryza_test_key', baseUrl: 'http://evil.com' })
      ).toThrow('baseUrl must use HTTPS')
    })

    it('[M1] allows http://localhost for development', () => {
      expect(
        () => new Kadryza({ apiKey: 'kadryza_test_key', baseUrl: 'http://localhost:8080' })
      ).not.toThrow()
    })

    it('[M2] rejects timeout <= 0', () => {
      expect(
        () => new Kadryza({ apiKey: 'kadryza_test_key', timeout: 0 })
      ).toThrow('timeout must be a positive number')
    })

    it('[M2] rejects negative timeout', () => {
      expect(
        () => new Kadryza({ apiKey: 'kadryza_test_key', timeout: -1 })
      ).toThrow('timeout must be a positive number')
    })

    it('strips trailing slashes from baseUrl', async () => {
      const c = new Kadryza({
        apiKey: 'kadryza_test_key',
  ...
  describe('error mapping', () => {
    it('maps 401 to KadryzaAuthError', async () => {
      globalThis.fetch = vi.fn().mockResolvedValue(
        mockResponse(401, errorBody('UNAUTHORIZED', 'Invalid API key'))
      )

      await expect(client.transactions.get('id')).rejects.toThrow(KadryzaAuthError)
    })

    it('maps 404 to KadryzaNotFoundError', async () => {
      globalThis.fetch = vi.fn().mockResolvedValue(
    ...
    it('maps 409 to KadryzaDuplicateError with redacted phone', async () => {
      const existingTx = {
        id: 'existing-tx',
        reference: 'order_1',
        status: 'SUCCESS',
    ...
    it('maps 422 to KadryzaValidationError with fields', async () => {
      globalThis.fetch = vi.fn().mockResolvedValue(
        mockResponse(
          422,
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KadryzaValidationError)
        expect((error as KadryzaValidationError).fields).toEqual({
    ...
    it('maps 503 to KadryzaGatewayUnavailableError', async () => {
      globalThis.fetch = vi.fn().mockResolvedValue(
      ...
      ).rejects.toThrow(KadryzaGatewayUnavailableError)
    })

    it('maps unknown status codes to generic KadryzaError', async () => {
      globalThis.fetch = vi.fn().mockResolvedValue(
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KadryzaError)
        expect((error as KadryzaError).code).toBe('INTERNAL_ERROR')
        expect((error as KadryzaError).statusCode).toBe(500)
      }
    })
    ...
    it('[H1] handles malformed error body without crashing', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KadryzaError)
        expect((error as KadryzaError).message).toContain('500')
      }
    })
    ...
    it('handles unparseable error body gracefully', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KadryzaError)
        expect((error as KadryzaError).code).toBe('UNKNOWN')
      }
    })
  })
  ...
  describe('success path', () => {
    it('[H2] throws INVALID_RESPONSE when 200 body is not JSON', async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KadryzaError)
        expect((error as KadryzaError).code).toBe('INVALID_RESPONSE')
        // Should NOT be KadryzaNetworkError
        expect(error).not.toBeInstanceOf(KadryzaNetworkError)
      }
    })
  })
  ...
  describe('timeout', () => {
    it('[C2] throws KadryzaNetworkError when request times out', async () => {
      const slowClient = new Kadryza({
        apiKey: 'kadryza_test_key',
        baseUrl: 'https://api.test.com',
        timeout: 50,
      })
    ...
    it('[H4] detects AbortError without instanceof DOMException', async () => {
      ...
      expect(error).toBeInstanceOf(KadryzaNetworkError)
      expect((error as KadryzaNetworkError).message).toContain('timed out')
    }, 10_000)
  })
  ...
  describe('network failure', () => {
    it('throws KadryzaNetworkError on fetch failure', async () => {
    ...
    it('includes original error message in KadryzaNetworkError', async () => {
      globalThis.fetch = vi.fn().mockRejectedValue(
        new TypeError('getaddrinfo ENOTFOUND api.test.kadryza.app')
      )
      ...
      try {
        ...
        expect(error).toBeInstanceOf(KadryzaNetworkError)
        expect((error as KadryzaNetworkError).message).toContain('ENOTFOUND')
      }
    })
  })
      ...
      expect(vi.mocked(globalThis.fetch)).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          headers: expect.objectContaining({
            'Authorization': 'Bearer kadryza_test_xxxxxxxxxxxxxxxxxxxx',
  ...
  describe('error serialization', () => {
    it('[M5] JSON.stringify preserves code and statusCode', () => {
      const error = new KadryzaAuthError('Invalid key')
      const json = JSON.parse(JSON.stringify(error))

      expect(json.name).toBe('KadryzaAuthError')
