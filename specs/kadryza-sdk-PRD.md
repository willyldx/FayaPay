...
> Ce projet fait partie du monorepo `kadryza/kadryza-sdk/`

---

## 1. Contexte du projet

`kadryza-sdk` est le package npm officiel que les merchants installent dans
leur projet pour intégrer Kadryza en quelques lignes de code.

Il est un simple wrapper typé autour de l'API REST `kadryza-backend`.
Zéro logique métier — uniquement des appels HTTP propres avec TypeScript.

**Objectif :** Un merchant doit pouvoir intégrer Kadryza en moins de 10 minutes.
...
│   ├── client.ts                 # Classe principale Kadryza
│   ├── resources/
...
```typescript
// Toutes les erreurs du SDK étendent KadryzaError
// pour que le merchant puisse les attraper facilement

export class KadryzaError extends Error {
  code: string
  statusCode?: number
}

export class KadryzaAuthError extends KadryzaError {
  // API key invalide ou expirée
  // code: 'UNAUTHORIZED'
}

export class KadryzaValidationError extends KadryzaError {
  // Paramètres invalides
  // code: 'VALIDATION_ERROR'
  fields?: Record<string, string>
}

export class KadryzaNotFoundError extends KadryzaError {
  // Transaction introuvable
  // code: 'NOT_FOUND'
}

export class KadryzaDuplicateError extends KadryzaError {
  // Référence déjà utilisée (idempotence)
  // code: 'DUPLICATE_REFERENCE'
  existingTransaction?: Transaction
}

export class KadryzaNetworkError extends KadryzaError {
  // Erreur réseau ou timeout
  // code: 'NETWORK_ERROR'
}

export class KadryzaGatewayUnavailableError extends KadryzaError {
...
import { verifyWebhookSignature, WebhookPayload } from '@kadryza/sdk'

const app = express()

// IMPORTANT : utiliser express.raw() pour avoir le body brut
app.post('/webhooks/kadryza',
...
export async function POST(req: NextRequest) {
  ...
  const signature = req.headers.get('x-kadryza-signature') ?? ''
...
2. Installation (`npm install @kadryza/sdk`)
...
5. **Idempotence visible** — KadryzaDuplicateError expose la transaction existante
