...
> Ce projet fait partie du monorepo `kadryza/kadryza-gateway/`

---

## 1. Contexte du projet

`kadryza-gateway` est une application Android native (Kotlin) qui tourne en permanence
sur un device Android physique dédié, branché en continu sur secteur.

Son rôle est d'être le pont physique entre le backend Kadryza et les opérateurs
...
Il communique avec `kadryza-backend` via une connexion WebSocket persistante et sécurisée.
...
│   │   │   ├── KadryzaGatewayApp.kt             # Application class (Koin init)
│   │   │   ├── core/
...
```kotlin
// Comportement attendu :
// - Connexion à wss://api.kadryza.app/v1/gateway/ws
...
// Paramètres > Accessibilité > Kadryza Gateway
...
## 6. Protocole des messages (reprend kadryza-backend PRD)
...
data class GatewayConfig(
    ...
    val backendUrl: String,       // "wss://api.kadryza.app/v1/gateway/ws"
...
BACKEND_WSS_URL = "wss://api.kadryza.app/v1/gateway/ws"
GATEWAY_TOKEN   = "<token fourni par kadryza-backend à la création du gateway>"
